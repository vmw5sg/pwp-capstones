...
def get_average_sentence_length(text):
    sentences_in_text = []
    words_in_sentences = []
    fix_punc = text.replace("?",".")
    text_2 = fix_punc.replace('!', '.')
    text_split = text_2.split('.')
    sentences_in_text.append(text_split)
    total_sent = len(text_split)
    for words in sentences_in_text:
        words_in_sentences.append(words)
        total_word = len(words_in_sentences) 
    print total_sent
    print total_word
    average = total_word/ total_sent       
    print average
...
class TextSample():
    ...
    def __repr__(self):
        return "The {author} has a {average} average sentance length".format(author=self.author, average=self.average_sentence_length)
...
def build_frequency_table(corpus):
    frequency_table = {}
    for an_element in corpus:
        if an_element in frequency_table.keys():
            frequencty_table[an_element] = frequency_table[an_element] + 1
        else:
            frequency_table[an_element] + 1
...
def ngram_creator(text_list):
    in_order1 = []
    in_order2 = []
    for pairs in range(len(text_list) -1): 
        in_order1 = text_list[pairs] +  " " + text_list[pairs+1]
        in_order2.append(in_order1)
    return in_order2
...
table1 = build_frequency_table(prepare_text(murder_note))
table2 = build_frequency_table(prepare_text(lily_trebuchet_intro))
...
def find_text_similarity(text_sample1, text_sample2):
    sentence_length_difference = percent_difference(text_sample1, text_sample2)
    sentence_length_similarity = abs(1-sentence_length_difference)
    
    word_count_similarity = frequency_comparison(table1, table2)
    ngram_similarity = murderer_sample.ngram_frequency() - lily_sample.ngram_frequency()
...
print(‘Lily Trebuchet’ + Lily)
Myrtle = find_text_similarity(murderer_sample, myrtle_sample)
print(‘Myrtle Beech’ + Myrtle)
Gregg = find_text_similarity(murderer_sample, gregg_sample)
print(‘Gregg T Fishy’ + Gregg)
